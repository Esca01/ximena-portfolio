...
(() => {
  ...
  function load() {
    ...
    // Outside the design runtime there is no sidecar to fetch — the slot is
    // purely src-driven (read-only). Skip the request so the live site makes
    // no failing call for .image-slots.state.json.
    if (!(window.omelette && window.omelette.writeFile)) {
      loaded = true; loadP = Promise.resolve(); subs.forEach((fn) => fn()); return loadP;
    }
